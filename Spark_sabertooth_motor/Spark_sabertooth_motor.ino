...
/*int left_vin = 26; // not using these sensors
int left_vo = 27;
int right_vin = 28;
int right_vo = 29;
*/
...
void loop() {
  ...
  if (checkCenterClear() && !newArray()) {
    motor.forward(30, 20); // move forward a little
    ...
    if (lowest > forward_thresh) {
      ...
      if (index < ARRAY_SIZE / 2) { // then lowest point is on the left
        motor.left(30, 150, STOP);
        shiftMapLeft();
      } else { // otherwise turn right
        motor.right(30, 150, STOP);
        //shiftMapRight();
      }
      if (servo_pos + servo_direction > 90 || servo_pos + servo_direction < 0) {
        servo_direction = -servo_direction;
...
/* Get the index of the argument in the ir_map array
 * returns a value starting with index 0
 */
int indexOf(int val) {
  for (int i = 0; i < ARRAY_SIZE; i++) {
...
/* Shifts the ir_map to the left by moving all the values by one 
 * bucket. The added bucket is replaced with a value 500 (very close)
 */
void shiftMapLeft() {
...
/* Shifts the ir_map to the right by moving all the values by one
 * bucket. The added bucket is replaced with a value 500 (very close)
 */
void shiftMapRight() {
...
/* Finds the minimum value in the ir_map
 */
int minimum() {
  int val = 500;
  for (int i = 0; i < ARRAY_SIZE; i++) {
